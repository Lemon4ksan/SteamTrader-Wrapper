...
class NoTradeLink(TradeError):
    """Класс исключения, вызываемый в случае, если у клиента нет ссылки на обмен."""

class NoSteamAPIKey(TradeError):
    """Класс исключения, вызываемый в случае, если у вас нет ключа Steam API."""
...
class ExpiredTradeLink(TradeError):
    """Класс исключения, вызываемый в случае, если ваша ссылка для обмена больше недействительна."""
...
class TradeBlockError(TradeError):
    """Класс исключения, вызываемый в случае, если у вас не включён Steam Guard или стоит блокировка обменов."""

class MissingRequiredItems(TradeError):
    """Класс исключения, вызываемый в случае, если в вашем инвентаре Steam отсутствуют необходимые для передачи предметы."""

class HiddenInventory(TradeError):
    """Класс исключения, вызываемый в случае, если ваш инвентарь скрыт."""
...
class NotEnoughMoney(TradeError):
    """Класс исключения, вызываемый в случае, если на балансе недостаточно средств для совершения покупки."""

class OfferCreationFail(TradeError):
    """Класс исключения, вызываемый в случае, если во время создания запроса, связанного с торговлей, произошла ошибка"""

class NetworkError(SteamTraderError):
    """Базовый класс исключений, вызываемых для ошибок, связанных с запросами к серверу."""

class OperationFail(NetworkError):
    """Класс представляющий исключения, вызываемый в случае, если запрсо был правильным, но операция не прошла успешно."""
...
class BadRequestError(NetworkError):
    """Класс исключения, вызываемый в случае отправки неправильного запроса. Код 400"""
